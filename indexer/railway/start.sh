...
#
# 4.14.x image layout: OPENSEARCH_PATH_CONF=/usr/share/wazuh-indexer/config,
# certs at config/certs/, security yml at config/opensearch-security/.
set -euo pipefail
...
CONF=/usr/share/wazuh-indexer/config

# --- certs -------------------------------------------------------------------
rm -rf "$CONF/certs"
mkdir -p "$CONF/certs"
python3 /railway/wazuh_certs.py indexer "$CONF/certs"

# --- opensearch.yml -----------------------------------------------------------
sed -e "s|__DOMAIN__|${RAILWAY_PRIVATE_DOMAIN}|g" \
  /railway/opensearch.yml.tpl > "$CONF/opensearch.yml"
...
INDEXER_PASSWORD="${INDEXER_PASSWORD}" DASHBOARD_PASSWORD="${DASHBOARD_PASSWORD}" \
  ...
  > "$CONF/opensearch-security/internal_users.yml"

# --- ownership/permissions for the indexer user (uid 1000) --------------------
chown -R 1000:0 "$CONF/certs" /usr/share/wazuh-indexer/logs \
  /var/lib/wazuh-indexer /var/log/wazuh-indexer /run/wazuh-indexer 2>/dev/null || true
chown 1000:0 "$CONF/opensearch.yml" "$CONF/opensearch-security/internal_users.yml"
chmod 700 "$CONF/certs"
chmod 400 "$CONF/certs"/*
chmod 600 "$CONF/opensearch.yml"
chmod 640 "$CONF/opensearch-security/internal_users.yml"
